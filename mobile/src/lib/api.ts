...
// Google-Drive-synced resources — a separate static snapshot (regenerated by
// a management command), not rows in the Resource DB table. This is what the
// web resources page actually shows as its main list; the DB-backed
// /resources/ endpoint above is only ever populated by student/admin
// submissions (its own "Community Submitted" section on web). Mobile needs
// both sources for parity, since /resources/ alone is often empty.
...
export const resourcesAPI = {
  ...
  getDriveResources: () => api.get<DriveResourceData[]>('/resources/drive/'),
